...
for filename in os.listdir('../run_results_sub/'):
...
recall_type=4
file_input=open('../fuse_recall_results_on_official_clusters_by_cluster_size_union_sub.txt','r')
...
recall=[]
precision=[]
file_input=open('../fuse_recall_results_on_official_clusters_by_cluster_size_intersection_sub.txt','r')
lines=file_input.readlines()
for i in range(1,len(lines)):
   line=lines[i].strip().split()
   if line[2]=="all":
...
file_input=open('fusion_union_sub_'+threshold+'_'+dimension+'.txt','r')
lines=file_input.readlines()
for i in range(1,len(lines)):
   line=lines[i].strip().split()
   if line[2]=="all":
...
ax.plot(recall, precision, 'bo',alpha=0.3,label=str('intersection '+threshold+' '+dimension))
ax.legend(loc=1,numpoints=1)
...
plt.savefig('Precision vs. weighted recall_single intersection complement '+dimension+' '+threshold+'.png')
plt.show()
plt.close()
