...
recall_path="fuse_recall_results_on_official_clusters_by_cluster_size_union_sub.txt"
...
for i in range(len(runtags)):
   for j in range(i+1,len(runtags)):
      ...
      os.system(str("python fuse_weighted_recall_by_cluster_size_union.py -q qrels2014.txt -c cluster_official.json -a "+runtags[i]+" -b "+runtags[j]+" -r "+recall_path))