...
parser.add_argument('-t', required=True, metavar='runtag',help='run tag')
...
runtag=vars(args)['t']
run_path = "run_results/"+runtag
if "official" in clusters_path:
   recall_path="recall_results_on_official_clusters_by_cluster_max/"+runtag+".txt"
elif "alternative" in clusters_path:
   recall_path="recall_results_on_alternative_clusters_by_cluster_max/"+runtag+".txt"
...
file_write.write("runtag".ljust(runlength) + "\ttopic\tunweighted_recall weighted_recall precision\n")
for topic in sorted(topics.keys()):
	...
	for cluster in clusters_dt[topic_ind]:
		...
		for tweet in cluster:
			weight = max(weight,int(qrels_dt[topic_ind][tweet]))
	...
	file_write.write(run_path[run_path.rindex("/") + 1:].ljust(max(runlength, 6)) + "\t" + "MB" + str(topic_ind) + "\t" + "%12.4f" % unweighted_recall + "\t" + "%12.4f" % weighted_recall + "\t" + "%10.4f" % precision+"\n")
...
file_write.write(run_path[run_path.rindex("/") + 1:].ljust(max(runlength, 6)) + "\t" + "all".ljust(5) + "\t" + "%12.4f" % unweighted_recall_mean + "\t" + "%12.4f" % weighted_recall_mean + "\t" + "%10.4f" % precision_mean+"\n")
file_run.close()
file_clusters.close()
file_write.close()
