...
if __name__=="__main__":
   ...
   print str(0)+": deduplication for that many pairs used "+str(timeit.default_timer() - start)+" seconds"
   recall_path_new=recall_path[0:recall_path.index(".txt")]+"_groundtruth.txt"
   ...
   for i in range(len(runtags)):
      for j in range(i+1,len(runtags)):
         ...
         calRecallPrecision(runtags[i],runtags[j],topics,file_write,tweetDict,word_vecs,0,int(dimension))
   file_write.close()
